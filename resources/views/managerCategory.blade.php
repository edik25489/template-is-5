    ...
    <div class="container">
        ...
        <table class="table">
            ...
            <tr class="pt-2">
                ...
                <td>
                    @if($item->id != 1)
                        <button type="button" class="btn btn-outline-warning" data-bs-toggle="modal" data-bs-target="#categoryEdit{{$item->id}}">
                        ...
                        <div class="modal fade" id="categoryEdit{{$item->id}}" tabindex="-1" aria-labelledby="categoryEditLabel{{$item->id}}" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h1 class=" modal-title fs-5" id="categoryEditLabel{{$item->id}}">Редактировать категорию</h1>
                                    </div>
                                    <form action="{{route('categoryEdit',['category'=>$item->id])}}" method="post" enctype="multipart/form-data">
                                        @csrf
                                        <div class="modal-body">
                                            <div class="mt-1">
                                                <label for="exampleEditName{{$item->id}}" class="form-label">Изменить название</label>
                                                <input name="name"
                                                       type="text"
                                                       class="form-control"
                                                       id="exampleEditName{{$item->id}}"
                                                       value="{{$item->name}}">
                                            </div>
                                            <div class="mt-2">
                                                <label for="categoryEditImage{{$item->id}}" class="form-label">Изменить изображение</label>
                                                <input
                                                    name="image"
                                                    type="file"
                                                    class="form-control"
                                                    id="categoryEditImage{{$item->id}}"
                                                    accept=".jpg, .jpeg, .png"
