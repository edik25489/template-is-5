                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                @if(\Illuminate\Support\Facades\Auth::guard('user')->check())
                                    <div class="btn-group">
                                            ...
                                            <form action="{{route('cartAddCount',['cart'=>$cart->id, 'currentRouteName'=> request()->route()->getName()])}}"
                                                  method="post">
                                                @csrf
                                            <button type="submit" class="btn btn-outline-primary">
                                        ...
                                        @endif
                                                ...
                                                @if(!\Illuminate\Support\Facades\Auth::guard('user')->user()->favorites->where('product_id','=',$item->id)->isEmpty())
                                                    <p class="d-none">
                                                        {{$favorites = \Illuminate\Support\Facades\Auth::guard('user')->user()->favorites->where('product_id','==',$item->id)->first()}}
                                                    </p>
                                                    <form action="{{route('favoritesDelete',['favorites'=>$favorites->id, 'currentRouteName'=> request()->route()->getName()])}}"
                                                          method="post">
                                                        @csrf
                                                ...
                                                @else
                                                    <form action="{{route('favoritesAdd',['product'=>$item->id, 'currentRouteName'=> request()->route()->getName()])}}"
                                                          method="post">
                                                        @csrf
                                                        <button type="submit" class="btn btn-outline-primary">
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-heart-fill" viewBox="0 0 16 16">
